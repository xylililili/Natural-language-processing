...
def generate_data_word2vec(
    ...
    # model = Word2Vec(h0_documents + h1_documents,
    ...
    X = np.array([document_vector(doc) for doc in h0_documents + h1_documents])
    y = np.array([0 for _ in h0_documents] + [1 for _ in h1_documents])
    return split_train_test(X, y)
